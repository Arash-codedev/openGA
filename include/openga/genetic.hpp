...
class Genetic {
  ...
  public:
    ...
    Genetic()
        : unif_dist(0.0, 1.0), N_robj(0), problem_mode(GA_MODE::SOGA),
          ...
          generation_max(100), tol_stall_average(1e-4), average_stall_max(10),
          tol_stall_best(1e-6), best_stall_max(10),
          ...
          calculate_SO_total_fitness(nullptr), calculate_MO_objectives(nullptr),
          distribution_objective_reductions(nullptr), init_genes(nullptr),
          eval_solution(nullptr), eval_solution_IGA(nullptr), mutate(nullptr),
          crossover(nullptr), SO_report_generation(nullptr),
          MO_report_generation(nullptr), custom_refresh(nullptr),
    ...
    void solve_init() {
        ...
        if (!is_single_objective()) {
            ...
            if (!reference_vector_divisions) {
                ...
                if (verbose) {
                    cout << "**************************************" << endl;
                    cout << "reference_vector_divisions: "
                         << reference_vector_divisions << endl;
                    cout << "**************************************" << endl;
                }
            }
        }
  ...
  protected:
    ...
    void check_settings() {
        if (is_interactive()) {
            ...
            if (calculate_SO_total_fitness != nullptr)
                throw runtime_error("calculate_SO_total_fitness is not null in "
                                    "interactive mode!");
            ...
            if (distribution_objective_reductions != nullptr)
                throw runtime_error("distribution_objective_reductions is not "
            ...
            if (eval_solution != nullptr)
                throw runtime_error("eval_solution is not null in interactive "
                                    "mode (use eval_solution_IGA instead)!");
        } else {
            if (calculate_IGA_total_fitness != nullptr)
                throw runtime_error("calculate_IGA_total_fitness is not null "
            ...
            if (is_single_objective()) {
                if (calculate_SO_total_fitness == nullptr)
                    throw runtime_error("calculate_SO_total_fitness is null in "
                                        "single objective mode!");
                if (calculate_MO_objectives != nullptr)
                    throw runtime_error("calculate_MO_objectives is not null "
                                        "in single objective mode!");
                if (distribution_objective_reductions != nullptr)
                    throw runtime_error("distribution_objective_reductions is "
                                        "not null in single objective mode!");
                if (MO_report_generation != nullptr)
                    throw runtime_error("MO_report_generation is not null in "
                                        "single objective mode!");
            } else {
                if (calculate_SO_total_fitness != nullptr)
                    throw runtime_error("calculate_SO_total_fitness is no null "
                                        "in multi-objective mode!");
                if (calculate_MO_objectives == nullptr)
                    throw runtime_error("calculate_MO_objectives is null in "
                                        "multi-objective mode!");
                // if(distribution_objective_reductions==nullptr)
                // 	throw runtime_error("distribution_objective_reductions
                // is null in multi-objective mode!");
                if (MO_report_generation == nullptr)
                    throw runtime_error("MO_report_generation is null in "
                                        "multi-objective mode!");
            }
        }
        ...
        if (is_single_objective()) { // SO (including IGA)
            if (SO_report_generation == nullptr)
                throw runtime_error("SO_report_generation is not adjusted "
                                    "while problem mode is single-objective");
            if (MO_report_generation != nullptr)
                throw runtime_error("MO_report_generation is adjusted while "
                                    "problem mode is single-objective");
        } else {
            if (SO_report_generation != nullptr)
                throw runtime_error("SO_report_generation is adjusted while "
                                    "problem mode is multi-objective");
            if (MO_report_generation == nullptr)
                throw runtime_error("MO_report_generation is not adjusted "
    ...
    void rank_population_MO(thisGenerationType& gen) {
        ...
        do {
            next_front.clear();
            vector<unsigned int>& last_front =
    ...
    void init_population(thisGenerationType& generation0) {
        ...
        if (verbose) {
            cout << "Initial population of " << population
                 << " was created with " << total_attempts << " attemps."
                 << endl;
        }
    }
    ...
    void crossover_and_mutation_range(thisGenerationType* p_new_generation,
        ...
        for (int i = x_index_begin; i <= x_index_end; i++)
            crossover_and_mutation_single(p_new_generation, pop_previous_size,
                                          i, &dummy);
        *active_thread = 0; // false
    }
    ...
    void crossover_and_mutation(thisGenerationType& new_generation) {
        ...
        if (generation_step <= 0)
            return;
        unsigned int N_add = (unsigned int)(std::round(double(population) *
                                                       (crossover_fraction)));
        unsigned int pop_previous_size =
            (unsigned int)new_generation.chromosomes.size();
        if (is_interactive()) {
            if (N_add + elite_count != population)
                throw runtime_error("In IGA mode, elite fraction + crossover "
        ...
        if (!multi_threading || N_threads == 1 || is_interactive()) {
            ...
            for (unsigned int i = 0; i < N_add && !user_request_stop; i++)
                crossover_and_mutation_single(&new_generation,
                                              pop_previous_size, -1, &dummy);
        } else {
